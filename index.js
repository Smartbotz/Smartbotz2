...
const starts = async (kurr = new WAConnection()) => {
	...
	console.log(color(figlet.textSync('SmartBotz', {
		font: 'Standard',
...
console.log(color('> HARI ','silver'), color(`${tampilHari}\n`,'mediumseagreen'))
	kurr.browserDescription = [ 'SmartBotz', 'ubuntu', '3.0' ]
...
kurr.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
   ...
   // owner
    ...
    const htod = "6281313499129@s.whatsapp.net"
	...
	try {
		...
		if (anu.action == 'add') {
			...
			try {
				...
				sendButImage(anu.jid, teks, `SMARTBOT`, img,but = [{buttonId:`Hello World`, 
			...
			try {
				...
				sendButImage(anu.jid, teks, `SMARTBOT`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `Good Bye`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
...
kurr.on('group-update', async (anu) => {
    ...
    kurr.on('chat-update', async (message) => {
        ...
        ownerNumber = ["6281313499129@s.whatsapp.net","6281313499129@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6281313499129@s.whatsapp.net"
